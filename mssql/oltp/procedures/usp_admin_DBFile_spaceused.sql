...
SET ANSI_NULLS ON 
GO
CREATE procedure [dbo].[usp_admin_DBFile_spaceused] --- 2003/05/19 14:00
@objname nvarchar(776) = null,		-- The object we want size on.
@updateusage varchar(5) = false		-- Param. for specifying that
					-- usage info. should be updated.
as
--------****Modification of sp_spaceused
...
begin
	...
	select 
		...
		datafile_size = (convert (dec (15,2),@dbsize)) 
			* 8192 / 1048576,
		'free_space' = (case when @dbsize >= @reservedpages then
			(convert (dec (15,2),@dbsize) - convert (dec (15,2),@reservedpages)) 
			* 8192 / 1048576 else 0 end),
		'percent_free' = ((case when @dbsize >= @reservedpages then
			(convert (dec (15,2),@dbsize) - convert (dec (15,2),@reservedpages)) 
			* 8192 / 1048576 else 0 end)/((convert (dec (15,2),@dbsize)) 
			* 8192 / 1048576))*100
...
--exec [dbo].[usp_admin_DBFile_spaceused]
