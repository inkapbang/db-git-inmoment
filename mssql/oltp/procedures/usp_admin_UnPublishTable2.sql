...
CREATE PROCEDURE [dbo].[usp_admin_UnPublishTable2]
	@article VARCHAR(100)
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				ELSE
				BEGIN
					PRINT 'Skipped unpublishing article "' + @article + '" because no publication exist for database in use.';
				END
			END
			ELSE
			BEGIN
				PRINT 'Skipped unpublishing article "' + @article + '" because database is not enabled for replication.';
			END
		END
		ELSE
		BEGIN
	...
	-- USAGE:
		--EXEC [dbo].[usp_admin_UnPublishTable2] 'Article'
	-- EXAMPLE:
		--EXEC [dbo].[usp_admin_UnPublishTable2] 'SurveyResponse'
END;
GO
