...
SET ANSI_NULLS ON 
GO
--ALTER usp_app_ModifyLocationCategory_3
CREATE PROC [dbo].[usp_app_ModifyLocationCategory_3](@objectId INT, @name nvarchar(100), @type INT, @parentId INT, @oldParentId INT, @rootId INT, @oldRootId INT, @orgId INT, @externalId varchar(100), @reviewOptIn bit, @reviewAggregate bit, @reviewUrl varchar(100))
AS
...
BEGIN	
	...
	ELSE
		BEGIN
			update LocationCategory set name=@name, externalId=@externalId, reviewOptIn=@reviewOptIn, reviewAggregate=@reviewAggregate, reviewUrl=@reviewUrl, locationCategoryTypeObjectId=@type, [version]=([version]+1) where objectId=@objectId;
			SET @updateTree = 0;
		END
