...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[sp_RedemptionCodeCustom_Date_Upload_AutoDelivery]
...
IF		@deliveryEmailCheck		= 0
	...
	BEGIN
		...
		PRINT CHAR(9) + 'Market'
		PRINT CHAR(9) + 'StartDate'
		PRINT CHAR(9) + 'EndDate'
		PRINT ''
		PRINT ''
		PRINT ''
...
855             0000749                 A&W 8		10/01/2015	10/30/2015
1044            Z005278n24              SLC			09/01/2015	09/30/2015
727             0011692001              WestVirgina	01/01/2015	01/30/2015
...
--Isa
--select *
--from _RedemptionCodeCustom_Pre_Upload
--Isa
...
BEGIN
	...
	SELECT
			...
			, t10.RedemptionCode
			, t10.Market
			, t10.usableStartDate  --Isa
			, t10.usableEndDate  --Isa
...
INSERT INTO _RedemptionCodeCustom_Upload ( OrganizationObjectId, RedemptionCode, Market, usableStartDate,usableEndDate )
SELECT
		t10.OrganizationObjectId
		, t10.RedemptionCode
		, t10.Market
...
BEGIN
	...
	--print 'list out exist file'
	--select * from _RedemptionCodeCustom_Upload_Exist  --Isa						
...
PRINT CHAR(9) +  'sp_RedemptionCodeCustom_Date_Upload_AutoDelivery	@answer = ''EXECUTE'''  --Isa
PRINT ''
PRINT ''
PRINT '*****  To terminate process  *****'
PRINT  CHAR(9) + 'sp_RedemptionCodeCustom_Date_Upload_AutoDelivery	@answer = ''terminate'''  --Isa
...
BEGIN
	...
	BEGIN
		...
		/********************  RedemptionCodeCustom Insert  ********************/  --Isa

		-----Cursor for RCC Insert

		DECLARE @countV2 bigint, @OrganizationObjectIdV2 int, @RedemptionCodeV2 varchar(50), @MarketV2 varchar(50), @LoadedDate dateTime, @UsableStartDateV2 dateTime, @UsableEndDateV2 dateTime
		...
		SELECT OrganizationObjectId, RedemptionCode, Market , usableStartDate,usableEndDate FROM _RedemptionCodeCustom_Upload_Insert

		OPEN mycursor
		FETCH next FROM mycursor INTO @OrganizationObjectIdV2, @RedemptionCodeV2, @MarketV2,@UsableStartDateV2,@UsableEndDateV2

		WHILE @@Fetch_Status = 0
		BEGIN

			  
		PRINT cast(@countV2 as varchar)+', '+cast(@OrganizationObjectIdV2 as varchar)+', '+cast(@RedemptionCodeV2 as varchar)+', '+cast(@MarketV2 as varchar)+', '+cast(@UsableStartDateV2 as varchar)+', '+cast(@UsableEndDateV2 as varchar)
		...
		INSERT INTO RedemptionCodeCustom ( OrganizationObjectId, RedemptionCode, Market, LoadedDate,usableStartDate,usableEndDate )
		SELECT @OrganizationObjectIdV2, @RedemptionCodeV2, @MarketV2, @LoadedDate,@UsableStartDateV2,@UsableEndDateV2
...
BEGIN
	INSERT INTO ##RedemptionCodeCustomStatus_Action ( Action, OrganizationObjectId, RedemptionCode, Market,usableStartDate,usableEndDate )
	SELECT 	
