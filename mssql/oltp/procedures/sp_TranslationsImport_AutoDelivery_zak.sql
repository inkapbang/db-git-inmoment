...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[sp_TranslationsImport_AutoDelivery_zak]
	@DisplayInfo		int 			= 1
	, @ShowChecks		int 			= 0
...
/*
	...
	sp_TranslationsImport_AutoDelivery_zak	@DisplayInfo=0,	@FileName =  'dummy'	
		
	sp_TranslationsImport_AutoDelivery_zak	@answer = 'EXECUTE'


  sp_TranslationsImport_AutoDelivery_zak	@answer = 'terminate'
	
*/
	
insert into ##CompanyTranslatedFields_Upload
select lsv.localizedStringObjectId, 'tr' as 'LocaleKey', s.tr as 'Value' 
from Tag t
join ___z_translation s on t.name = s.en_us
join LocalizedStringValue lsv on t.labelObjectId = lsv.localizedStringObjectId and lsv.localeKey = 'en_us' --and lsv.value = 'Business Location'
...
--return	
-- Testing	
--SELECT *	FROM ##CompanyTranslatedFields_Upload where localizedStringObjectId = 20198673

--exec xp_cmdShell 'copy c:\tmpUpload\de_de.csv c:\tmpUpload\de_de.txt'

--exec xp_cmdshell 'dir c:\tmpUpload\de_de.*'
...
--, @copy_recipients 				= 'zbelghali@InMoment.com'
--, @reply_to						= 'zbelghali@InMoment.com'
