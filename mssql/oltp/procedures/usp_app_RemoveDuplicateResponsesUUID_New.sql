...
CREATE PROCEDURE [dbo].[usp_app_RemoveDuplicateResponsesUUID_New]
...
/*****************************  usp_app_RemoveDuplicateResponsesUUID  *****************************
	...
	2015.11.20 IKB	- Adding "lastModTime" filter
					- Not replicated stored procedure anymore

**************************************************************************************************/
DECLARE @CutoffTime DATETIME,@recordCount INT

SET @CutoffTime = GETDATE()


SET		@recordCount		= ( SELECT SUM(Duplicates) FROM ( SELECT COUNT(1) AS Duplicates 	FROM SurveyResponse WITH (NOLOCK) 	WHERE (lastModTime >= DATEADD(MINUTE,-20, @CutoffTime)) AND exclusionReason = 0 AND complete = 1 AND UUID IS NOT NULL 	GROUP BY uuid 	HAVING COUNT(1) > 1) AS t10		)
...
BEGIN
		...
		WHERE	
				(lastModTime >= DATEADD(MINUTE,-20,@CutoffTime))
			AND 
				exclusionReason = 0 
			AND 
				complete = 1
