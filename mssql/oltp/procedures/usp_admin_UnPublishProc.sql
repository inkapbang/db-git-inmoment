...
CREATE PROCEDURE [dbo].[usp_admin_UnPublishProc]
...
BEGIN
	...
	BEGIN
		...
		ELSE
		BEGIN
			RAISERROR('Failed to unpublish procedure "%s": Publication "%s" does not exist.', 16, 1, @article, @publication);
		END
	END
	...
	BEGIN
		...
		BEGIN
			...
			-- Set the output
			IF EXISTS (SELECT * FROM @IsPublishedProc2 where [article name] = @article)
			BEGIN
				IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[' + CAST(@article as nvarchar(max)) + ']') AND type in (N'P', N'PC'))
				BEGIN
					PRINT 'Unpublishing procedure "' + @article + '" from publication "' + @publication + '"...';
	...
	-- USAGE:
		--EXEC [dbo].[usp_admin_UnPublishProc] 'Publication', 'Procedure'
	-- EXAMPLE:
		--EXEC [dbo].[usp_admin_UnPublishProc] 'LindaOLTP010111Pub', 'MyStoredProc'
END;
GO
