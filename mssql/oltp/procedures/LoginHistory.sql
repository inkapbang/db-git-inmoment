...
SET ANSI_NULLS ON 
GO
CREATE procedure [dbo].[LoginHistory]
(
...
/*
Usage:
exec loginHistory 2575 -- 1187  --2575
*/
	...
	join AccessEventLog ael on ael.userAccountObjectId = ua.objectId
	where 1=1
	...
	and ua.email not in (select [email] from login_history_email_exclusion where org_id = @organizationObjectId)
	...
	from #userRoles t
	join #LoginHistory h on t.objectId=h.objectId ) usr
