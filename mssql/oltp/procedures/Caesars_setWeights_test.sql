...
SET ANSI_NULLS ON 
GO
CREATE procedure [dbo].[Caesars_setWeights_test]
(
	@fdt datetime = null
	, @tdt datetime = null
)
...
/*
	...
	exec Caesars_setWeights_test '2017-07-10', '2017-07-10'
...
begin
  ...
  set @now = CAST(convert(varchar(10), getdate(), 112) as datetime)
	...
	select @fdt, @tdt, datepart(dw, @now)
  ...
  select  sr.objectid, sr.offercode, dfo.name--, dfo.objectId
	into __weight__
  ...
  select count(*) from __weight__
  select * from surveyMix
