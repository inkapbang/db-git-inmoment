...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[sp_SurveyResponseAlert_Remove_AutoDelivery_Unmigrated]
...
IF		@deliveryEmailCheck		= 0	
	...
	BEGIN
		...
		PRINT CHAR(9) + 'sp_SurveyResponseAlert_Remove_AutoDelivery_Unmigrated ''Their Email Here'''
...
sp_SurveyResponseAlert_Remove_AutoDelivery_Unmigrated
...
IF 
	EXISTS (
		SELECT 1 
		FROM 
			_surveyResponseAlert_Removal x
		inner join #tmp_ODS_ORGS ods with (nolock) on x.OrgId = ods.organizationObjectId
		--inner join _ODS_ORGS ods with (nolock) on x.OrgId = ods.organizationObjectId
	)
BEGIN
...
PRINT CHAR(9) +  'sp_SurveyResponseAlert_Remove_AutoDelivery_Unmigrated 	@answer = ''EXECUTE'''
PRINT ''
PRINT ''
PRINT '*****  To terminate process  *****'
PRINT CHAR(9) +  'sp_SurveyResponseAlert_Remove_AutoDelivery_Unmigrated 	@answer = ''terminate'''
