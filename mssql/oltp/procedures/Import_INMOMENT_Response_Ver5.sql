...
-- =============================================
CREATE PROCEDURE [dbo].[Import_INMOMENT_Response_Ver5]
@OrganizationId int,
...
/*
	...
	[locationObjectId],
	[offerObjectId],
	[offerCode],[reviewOptin],modeType,responseSourceObjectId
	)
	...
	select	@SurveyLang = Lang from putwh06.Import.dbo.ResponseHeader_INMOMENT r with (nolock) 
	where	r.ResponseId = @ResponseId and r.OrganizationId = @OrganizationId and r.MS_SurveyResponseObjectId is null
	...
	select	@MS_SurveyResponseObjectId,
			...
			numericValue,
			case when fieldType in (2,164,162,163,165,166) then substring(textValue,1,200) else NULL end as textValue,
			dateValue,
			booleanValue,
			1,
			dataFieldObjectId,
	...
	--COMMIT TRANSACTION IMPORT_RESPONSE_ZIOSK
*/
	return 0
