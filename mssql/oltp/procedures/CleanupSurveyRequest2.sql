...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[CleanupSurveyRequest2] as
begin

--exec [dbo].[CleanupSurveyRequest2]

	declare @sr table(srid int)
	insert into @sr 
	select objectid from surveyrequest where purgeTime < GetDate()
	--select * from @sr
...
---
	...
	insert into @sr2 
	select objectid from surveyrequest where (not state = 3) and expirationTime < GETDATE()
