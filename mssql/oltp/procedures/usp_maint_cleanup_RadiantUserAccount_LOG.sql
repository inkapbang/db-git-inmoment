...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[usp_maint_cleanup_RadiantUserAccount_LOG]
@CutoffNumDay INT = 60
AS
SET NOCOUNT ON;
...
SELECT  RLICHANGEID
FROM    [rli_con].[RadiantUserAccount_LOG]
...
BEGIN
    DELETE FROM [rli_con].[RadiantUserAccount_LOG] WHERE RLICHANGEID = @ID_TO_PURGE
    FETCH NEXT FROM RLI_PURGE_CUR INTO @ID_TO_PURGE
END
CLOSE RLI_PURGE_CUR
