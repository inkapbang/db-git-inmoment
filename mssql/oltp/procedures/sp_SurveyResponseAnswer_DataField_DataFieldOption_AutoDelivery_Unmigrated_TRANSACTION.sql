...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[sp_SurveyResponseAnswer_DataField_DataFieldOption_AutoDelivery_Unmigrated_TRANSACTION]
...
/****************  BackFill Options Auto Deliver  ****************
	...
	sp_SurveyResponseAnswer_DataField_DataFieldOption_AutoDelivery_Unmigrated		
...
IF		@deliveryEmailCheck		= 0
	...
	BEGIN
		...
		PRINT ''
		PRINT CHAR(9) + 'sp_SurveyResponseAnswer_DataField_DataFieldOption_AutoDelivery_Unmigrated'
...
IF 
	EXISTS (
		SELECT 1 
		FROM 
			_surveyResponseAnswer_DataField_DataFieldOption x
...
BEGIN
	...
	DROP TABLE _surveyResponseAnswer_DataField_DataFieldOption
	RETURN
END
...
PRINT CHAR(9) +  'sp_SurveyResponseAnswer_DataField_DataFieldOption_AutoDelivery_Unmigrated_TRANSACTION	@answer = ''EXECUTE'''
PRINT ''
PRINT ''
PRINT '*****  To terminate process  *****'
PRINT  CHAR(9) + 'sp_SurveyResponseAnswer_DataField_DataFieldOption_AutoDelivery_Unmigrated_TRANSACTION	@answer = ''terminate'''
...
BEGIN
	...
	IF (@sraInsertCheck > 0 or @sraUpdateCheck > 0)
	BEGIN



		PRINT 'Inserting/Updating Rows In Transaction'
		...
		DECLARE @InsertCnt bigint, @UpdateCnt bigint, @SurveyResponseId bigint, @DataFieldOptionId bigint
		DECLARE @InsertCntTotal bigint, @UpdateCntTotal bigint
		SET @InsertCntTotal = 0
		SET @UpdateCntTotal = 0

		CREATE TABLE _surveyResponseAnswer_DataField_DataFieldOption_Merged (SurveyResponseId bigint, UpdateCnt int, InsertCnt int)
		
		IF @sraUpdateCheck > 0
			INSERT INTO _surveyResponseAnswer_DataField_DataFieldOption_Merged
			SELECT	SurveyResponseId,count(*),0
			FROM	_surveyResponseAnswer_DataField_DataFieldOption_Update
			GROUP BY SurveyResponseId
			
		IF @sraInsertCheck > 0
			INSERT INTO _surveyResponseAnswer_DataField_DataFieldOption_Merged
			SELECT	SurveyResponseId,0,count(*)
			FROM	_surveyResponseAnswer_DataField_DataFieldOption_Insert
			GROUP BY SurveyResponseId	
						
	
			
		DECLARE mycursor CURSOR FOR
		SELECT	SurveyResponseId, UpdateCnt, InsertCnt
		FROM	_surveyResponseAnswer_DataField_DataFieldOption_Merged
		ORDER BY SurveyResponseId

		OPEN mycursor
		FETCH NEXT FROM mycursor INTO @SurveyResponseId,@UpdateCnt,@InsertCnt

		WHILE @@FETCH_STATUS = 0
		BEGIN
			PRINT 'BEGIN TRANSACTION FOR RESPONSEID '+CONVERT(VARCHAR(100),@SurveyResponseId)
			BEGIN TRANSACTION SRA_DFO_BACKFILL
			
			IF @UpdateCnt > 0
			BEGIN
				UPDATE SurveyResponseAnswer WITH (ROWLOCK)
				SET dataFieldOptionObjectId = u.DataFieldOptionId
				FROM _surveyResponseAnswer_DataField_DataFieldOption_Update u WITH (NOLOCK)
				WHERE SurveyResponseAnswer.objectId = u.SurveyResponseAnswerId
						AND u.SurveyResponseId = @SurveyResponseId


				SET @UpdateCntTotal = @UpdateCntTotal + @UpdateCnt
			END
			
			IF @InsertCnt > 0
			BEGIN
				DECLARE @maxSequence INT
				SELECT	@maxSequence = max(sequence) from SurveyResponseAnswer WITH (NOLOCK) 
				WHERE	surveyResponseObjectId = @SurveyResponseId

				INSERT INTO SurveyResponseAnswer (surveyResponseObjectId,dataFieldObjectId,dataFieldOptionObjectId,sequence)
				SELECT	SurveyResponseId,DataFieldId,DataFieldOptionId,@maxSequence + DENSE_RANK() OVER (ORDER BY DataFieldId ASC) AS sequence
				FROM	_surveyResponseAnswer_DataField_DataFieldOption_Insert i WITH (NOLOCK)
				WHERE	i.SurveyResponseId = @SurveyResponseId

				SET @InsertCntTotal = @InsertCntTotal + @InsertCnt
			END
			
			COMMIT TRANSACTION SRA_DFO_BACKFILL
		...
		PRINT ''			
		PRINT 'Insert/Update Portion Complete'
		PRINT ''
		PRINT ''
...
CLEANUP:
	...
	IF EXISTS(SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'_surveyResponseAnswer_DataField_DataFieldOption_Statistics') AND type = (N'U'))				DROP TABLE _surveyResponseAnswer_DataField_DataFieldOption_Statistics
	IF EXISTS(SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'_surveyResponseAnswer_DataField_DataFieldOption_InsertCompleted') AND type = (N'U'))		DROP TABLE _surveyResponseAnswer_DataField_DataFieldOption_InsertCompleted
	IF EXISTS(SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'_surveyResponseAnswer_DataField_DataFieldOption_Merged') AND type = (N'U'))						DROP TABLE _surveyResponseAnswer_DataField_DataFieldOption_Merged
