...
-- =============================================
CREATE PROCEDURE [dbo].[Import_EmpathicaData_Response_Ver5_Dynamite_UTC]
@OrganizationId int,
@ClientId int,
@ResponseId int,
...
/*
	...
	declare @EmpCode varchar(50), @locId int
	select @EmpCode = EmployeeCode_Answer,@locId = MS_locationId from putwh06.import.dbo.ResponseHeader with (nolock) where OrganizationId = @OrganizationId and ClientId = @ClientId and ResponseId = @ResponseId

	if not exists (select null from Employee where locationObjectId = @locId and employeeCode = @EmpCode)
	begin
		INSERT INTO dbo.Employee (locationObjectId,employeeCode,[version])
			values (@locId,@EmpCode,0)
		if @@error <> 0 or @@rowcount = 0 or @@rowcount > 1
		begin
			print 'Error Inserting To Employee'
	...
	[locationObjectId],
	[offerObjectId],
	[offerCode],[reviewOptin],modeType,externalId,responseSourceObjectId,loyaltyNumber,employeeCode)
	...
	select	r.MS_SurveyGatewayId,
			...
			@ResponseSourceId,
			r.CertCode,
			r.EmployeeCode_Answer
	from	putwh06.Import.dbo.ResponseHeader r with (nolock)
			inner join mindshare.dbo.OfferCode c with (nolock) on (r.MS_locationId = c.locationObjectId and r.MS_OfferId = c.offerObjectId and r.MS_SurveyGatewayId = c.surveyGatewayObjectId and r.unitcode = c.offercode)
	...
	select	@MS_SurveyResponseObjectId,
			...
			case when MS_FieldType in (2,164,162,163,165,166) then substring(AnswerValue,1,200) else NULL end as textValue,
...
where	rda.OrganizationId = @OrganizationId and rda.ClientId = @ClientId
	...
	union all
	-- Look for questions to duplicate
	select	rd.OrganizationId,
			rd.ResponseId,
			dm.MS_FieldType,
			dm.QType,
			...
			dm.MS_DataFieldId,
			dm.MS_DataFieldOptionId
	from	putwh06.Import.dbo.DataFieldMap_Add dm with (nolock)
			inner join putwh06.Import.dbo.ResponseDetailChoice rd with (nolock) on (dm.OrganizationId = rd.OrganizationId and dm.ClientId = rd.ClientId and dm.QuestionId = rd.QuestionId and dm.AnswerId = rd.AnswerId and dm.SurveyId = @SurveyId)
	where	rd.ResponseId = @ResponseId and rd.OrganizationId = @OrganizationId and rd.ClientId = @ClientId
			and dm.MS_DataFieldId is not null and dm.MS_FieldType <> 25
