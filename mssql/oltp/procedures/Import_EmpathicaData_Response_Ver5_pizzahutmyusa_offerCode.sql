...
-- =============================================
CREATE PROCEDURE [dbo].[Import_EmpathicaData_Response_Ver5_pizzahutmyusa_offerCode]
@OrganizationId int,
@ClientId int,
@ResponseId int,
...
where	rda.OrganizationId = @OrganizationId and rda.ClientId = @ClientId
	...
	select	rd.OrganizationId,
			...
			---1,
			---1,
			rd.dataFieldObjectId,
			rd.dataFieldOptionObjectId
	from	putwh06.Import.dbo.ResponseDetailChoiceCustom rd with (nolock)
			inner join DataField d with (nolock) on (rd.OrganizationId = d.organizationObjectId and rd.dataFieldObjectId = d.objectId)
	where	rd.ResponseId = @ResponseId and rd.OrganizationId = @OrganizationId and rd.ClientId = @ClientId
			and rd.dataFieldOptionObjectId is not null
