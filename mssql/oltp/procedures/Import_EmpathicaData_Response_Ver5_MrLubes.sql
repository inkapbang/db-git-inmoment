...
-- =============================================
CREATE PROCEDURE [dbo].[Import_EmpathicaData_Response_Ver5_MrLubes]
@OrganizationId int,
@ClientId int,
@ResponseId int,
...
/*
	...
	[locationObjectId],
	[offerObjectId],
	[offerCode],[reviewOptin],modeType,externalId)
	...
	select	r.MS_SurveyGatewayId,
			...
			1,
			-- Survey ID:	Flagship Import 8984 
			-- Offer ID:	Flagship Offer Import	5428 
			-- Gateway:		Flagship Import	5358
			
			-- Survey ID:	Walmart Import 8985 
			-- Offer ID:	Walmart Offer Import	5429 
			-- Gateway:		Walmart Import 5360 
 
			--sot.MS_SurveyId,
			case
				when r.MS_OfferId = 5428 and r.MS_SurveyGatewayID = 5358 then 8984
				when r.MS_OfferId = 5429 and r.MS_SurveyGatewayID = 5360 then 8985
				else 8984
			end as MS_SurveyId, 
			1,
			1,
...
where	rda.OrganizationId = @OrganizationId and rda.ClientId = @ClientId
	...
	begin
		...
		SELECT	1,ID_Filter.objectId,0,Answer_Filter.AnswerValue,datalength(Answer_Filter.AnswerValue),len(Answer_Filter.AnswerValue),0,0,case @SurveyLang when 'EN' then 1 else NULL end as commentLanguage
		FROM
		(
