...
-- =============================================
CREATE PROCEDURE [dbo].[Import_EmpathicaData_Response_Ver5_cantire]
@OrganizationId int,
@ClientId int,
@ResponseId int,
...
/*
	...
	select	r.MS_SurveyGatewayId,
			...
			1,
			case when r.MS_offerId = 5292 then 8766
				 when r.MS_offerId = 5293 then 8767
				 when r.MS_offerId = 5304 then 8772 
				 else NULL end as SurveyObjectId,
			1,
			1,
	...
	select	@MS_SurveyResponseObjectId,
			...
			case when MS_FieldType in (2,164,162,163,165) then substring(AnswerValue,1,200) else NULL end as textValue,
...
where	rda.OrganizationId = @OrganizationId and rda.ClientId = @ClientId
	...
	begin
		...
		SELECT	1,ID_Filter.objectId,0,Answer_Filter.AnswerValue,datalength(Answer_Filter.AnswerValue),len(Answer_Filter.AnswerValue),0,0,case @SurveyLang when 'EN' then 1 when 'FR' then 2 else NULL end as commentLanguage
		FROM
		(
