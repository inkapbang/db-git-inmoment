...
GO
/*  Apr 12, 2016 IW 
	Per discussion with Bob, we will -1 step on any response which is landed on "Exported to Empathica".
	Apr 14 2016 IW
	Added deviceType per request by Chris DBA-2333
*/


CREATE Procedure [dbo].[sp_DropOutAnalysis_AutoDelivery_PizzaHut]
...
sp_DropOutAnalysis_AutoDelivery
	-- required
...
BEGIN
	...
	SELECT 
			...
			, offercode
			, o.name 										AS offerName
			, sr.deviceTypeValue AS deviceType --Isa
...
--Isa
	--select * from ##dropOutAnalysis_02 where surveyResponseObjectId in (1795020286,1795059762) --For demo only

	delete ##dropOutAnalysis_02 where rowNumber = 1 and dataFieldObjectId=116252
	
	IF OBJECT_ID('tempdb..##dropOutAnalysis_02_new') IS NOT NULL	DROP TABLE ##dropOutAnalysis_02_new
	
	SELECT
		surveyResponseObjectId
		,objectId
		, name
		, sequence, rank() over(partition by surveyResponseObjectId order by sequence desc) AS rowNumber
		, dataFieldObjectId
		, dataFieldOptionObjectId 
	into ##dropOutAnalysis_02_new	
	from ##dropOutAnalysis_02
	--where surveyResponseObjectId in (1795020286,1795059762) --For demo only
--End Isa	
	...
	FROM 
		...
		(
			SELECT 
				...
				, dataFieldObjectId
				, dataFieldOptionObjectId
			FROM ##dropOutAnalysis_02_new --Isa 
			WHERE rowNumber = 1 			
		)as b 
			ON a.objectId=b.surveyResponseObjectId
			
		-- Left join will include all answers
		LEFT JOIN
...
IF		@deliveryEmailCheck	= 1
	...
	BEGIN
		...
		SELECT 
				objectId								AS SurveyResponseObjectId
				, SurveyObjectId
				, SurveyGatewayObjectId	
				, ModeType
				, REPLACE( Location		, ',', '' )		AS Location
				, REPLACE( SurveyName	, ',', '' )		AS SurveyName
				, BeginDate
				, BeginTime
				, Minutes
				, Ani
				, REPLACE( OfferCode	, ',', '' )		AS OfferCode	
				, REPLACE( OfferName	, ',', '' )		AS OfferName
				, REPLACE( PromptName	, ',', '' )		AS PromptName
				, DataFieldObjectId
		...
		FROM 
				##dropOutAnalysis_03
		--where ObjectId in (1795020286,1795059762)  --Isa  Demo only
...
--select * from ##dropOutAnalysis_ResultSet where SurveyResponseObjectId in (1795020286,1795059762)  --Isa  Demo only
...
SELECT * 
FROM ##dropOutAnalysis_ResultSet order by BeginDate,BeginTime
		
IF OBJECT_ID('tempdb..##dropOutAnalysis_01') IS NOT NULL		DROP TABLE ##dropOutAnalysis_01
IF OBJECT_ID('tempdb..##dropOutAnalysis_02') IS NOT NULL		DROP TABLE ##dropOutAnalysis_02
IF OBJECT_ID('tempdb..##dropOutAnalysis_02_new') IS NOT NULL	DROP TABLE ##dropOutAnalysis_02_new  --Isa
IF OBJECT_ID('tempdb..##dropOutAnalysis_03') IS NOT NULL		DROP TABLE ##dropOutAnalysis_03
IF OBJECT_ID('tempdb..##dropOutAnalysis_Results') IS NOT NULL	DROP TABLE ##dropOutAnalysis_Results	
IF OBJECT_ID('tempdb..##dropOutAnalysis_ResultSet') IS NOT NULL	DROP TABLE ##dropOutAnalysis_ResultSet	
