...
SET ANSI_NULLS ON 
GO
CREATE PROCEDURE [dbo].[sp_TranslationsImport_AutoDelivery_NoDelay]
	@DisplayInfo		int 			= 1
	, @ShowChecks		int 			= 0
...
SET		@FileNameBulkInsertStatement	= 'BULK INSERT ##CompanyTranslatedFields_Upload   	FROM ''C:\TmpUpload\' + @FileName + '''	  WITH ( FIRSTROW = 2, FIELDTERMINATOR = ''\t'',  DATAFILETYPE =	''widechar''   )'
...
PRINT CHAR(9) +  'sp_TranslationsImport_AutoDelivery_NoDelay	@answer = ''EXECUTE'''
PRINT ''
PRINT ''
PRINT '*****  To terminate process  *****'
PRINT  CHAR(9) + 'sp_TranslationsImport_AutoDelivery_NoDelay	@answer = ''terminate'''
