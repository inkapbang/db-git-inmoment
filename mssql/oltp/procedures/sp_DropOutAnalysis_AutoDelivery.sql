...
SET ANSI_NULLS ON 
GO
CREATE Procedure [dbo].[sp_DropOutAnalysis_AutoDelivery]
		@deliveryEmail	varchar(100)	= 'dba@inmoment.com'
		, @OrgId		int				= NULL
...
IF EXISTS(SELECT 1 FROM sys.objects WHERE OBJECT_ID = OBJECT_ID(N'_DropOutAnalysisReport') AND type = (N'U'))    DROP TABLE _DropOutAnalysisReport
...
, @query_attachment_filename	= @FileName			--Isa
...
PRINT 'If ani column is not null, please send the file into Inmoment ftp site otherwise you can send it through the email'			
