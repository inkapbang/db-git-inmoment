...
CREATE PROCEDURE [dbo].[usp_admin_UnPublishTable]
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			-- Set the output
			IF EXISTS (SELECT * FROM @IsPublishedTable where [article name] = @article)
			BEGIN
				IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[' + CAST(@article as nvarchar(max)) + ']') AND type in (N'U'))
				BEGIN
					PRINT 'Unpublishing article "' + @article + '" from publication "' + @publication + '"...';
					EXEC dbo.UnPublishTable @publication, @article;
					PRINT 'Article "' + @article + '" unpublished.';
				END
				ELSE
				BEGIN
					RAISERROR('Failed to unpublish article "%s": Table does not exist.', 16, 1, @article);
				END
			END
			ELSE
			BEGIN
				PRINT 'Skipped unpublish article "' + @article + '": Article "' + @article + '" does not exists in publication "' + @publication + '".';
			END
		END
		ELSE
		BEGIN
			RAISERROR('Failed to unpublish article "%s": Publication "%s" does not exist.', 16, 1, @article, @publication);
		END
	END
	...
	BEGIN
		...
		-- Populate the table with the entry matching the publication and article (if any)
		BEGIN TRY
			INSERT INTO @IsPublishedDatabase EXEC sp_helpPublication;
		END TRY
		BEGIN CATCH
		END CATCH
		...
		BEGIN
			...
			-- Declare the table variable to hold the results of the 
			-- sp_helpArticle procedure
			DECLARE @IsPublishedTable2 TABLE (
				[article id]					INT,
				[article name]					SYSNAME NULL,
				[base object]					NVARCHAR(257) NULL,
				[destination object]			SYSNAME NULL,
			...
			-- Set the output
			IF EXISTS (SELECT * FROM @IsPublishedTable2 where [article name] = @article)
			BEGIN
				IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[' + CAST(@article as nvarchar(max)) + ']') AND type in (N'U'))
				BEGIN
					PRINT 'Unpublishing article "' + @article + '" from publication "' + @publication + '"...';
	...
	-- USAGE:
		--EXEC [dbo].[usp_admin_UnPublishTable] 'Publication', 'Article'
	-- EXAMPLE:
		--EXEC [dbo].[usp_admin_UnPublishTable] 'LindaOLTP010111Pub', 'SurveyResponse'
END;
GO
